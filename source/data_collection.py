...
def get_top_news_topics(city):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for article in articles[:10]:  # Extract top 5 articles
...
def save_data(city_name):
    news_topics = get_top_news_topics(city_name)
    if news_topics:
        reddit = praw.Reddit(user_agent=True, client_id="dmoPjIG2Htcj5fXH8o9ufA", 
                            client_secret="cA_5xyFiuEOJ4AuGmbLd82GnOTk-qA", username='abhinash-', password='Abhinash143')
        ...
        for topic in news_topics:
            ...
            for subreddit_name in subreddit_names:
                ...
                search_results = subreddit.search(topic, sort="new", limit=10, time_filter="week")  # Search top 15 latest posts
                for submission in search_results:
                    comments = fetch_comments_from_reddit(submission.id)
                    if comments:
                        # Classify comments into positive, negative, neutral
                        positive_comments, negative_comments, neutral_comments = identify_viewpoints(comments[-4:])
